...
## ダウンロード
### ⬇️ [**light_rumor v1.0.2 をダウンロード（APK）**](https://github.com/minetan1229/light_rumor/raw/master/dist/light_rumor-v1.0.2.apk)

**最新バージョン: v1.0.2 更新内容**
- **UIの日本語化＆操作性向上**: カリング画面や現像スタジオのボタン・ラベル表記を整理し、直感的なワークフローを実現
- **最近編集した写真のカタログ・履歴管理**: 編集したRAW/写真のパラメータ保持および履歴からのワンタップ再開に対応
- **EXIF・メタデータ編集＆XMPサイドカー同期**: 撮影日時やカメラ情報のインライン編集と自動XMP保存の強化
- **星景追尾スタック＆画像パイプラインの高速化**: メモリ割り当て最適化（Kappa-Sigmaクリッピング等）により処理性能を向上
- **全画面Edge-to-Edge表示対応**: システムバー領域までシームレスに広がるモダンな没入体験を提供
