...
public:
    ...
    // Fast embedded thumbnail extraction for RAW and standard files
    // Extracts JPEG preview bytes without full debayer pipeline, achieving <10ms response
    static bool extractEmbeddedThumbnail(const std::string& filePath,
                                         std::vector<uint8_t>& outJpegBytes,
                                         int32_t& outWidth,
                                         int32_t& outHeight);

    static bool extractEmbeddedThumbnailFromBuffer(const uint8_t* data,
                                                   size_t size,
                                                   std::vector<uint8_t>& outJpegBytes,
                                                   int32_t& outWidth,
                                                   int32_t& outHeight);
