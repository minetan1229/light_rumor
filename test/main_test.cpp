...
bool testOutputSuccess() {
    ...
    APEX_TEST_ASSERT(genOk, "Failed to generate synthetic RAW");
    ...
    apex::ExportOptions jpegOpt;
    jpegOpt.format = apex::ExportFormat::JPEG;
    jpegOpt.jpegQuality = 98;
    jpegOpt.chromaSubsampling = apex::ChromaSubsampling::YUV444;
    ...
    APEX_TEST_ASSERT(jpgOk, "JPEG export failed");

    // Verify JPEG file and headers
    std::ifstream jf(jpegPath, std::ios::binary | std::ios::ate);
    APEX_TEST_ASSERT(jf.is_open(), "Could not open generated JPEG");
    size_t jfSize = static_cast<size_t>(jf.tellg());
    APEX_TEST_ASSERT(jfSize > 50000, "Generated JPEG file too small");
    jf.seekg(0);
    uint8_t jHead[4];
    jf.read(reinterpret_cast<char*>(jHead), 4);
    jf.close();
    // Check SOI marker 0xFFD8 and APP1 0xFFE1
    APEX_TEST_ASSERT(jHead[0] == 0xFF && jHead[1] == 0xD8, "JPEG SOI marker invalid");
    APEX_TEST_ASSERT(jHead[2] == 0xFF && jHead[3] == 0xE1, "JPEG Exif APP1 marker invalid");
    std::cout << "  ✓ Generated JPEG valid! File size: " << jfSize / 1024 << " KB\n";

    // 2. Export 16-bit TIFF
    apex::ExportOptions tiffOpt;
    tiffOpt.format = apex::ExportFormat::TIFF16;
    ...
    APEX_TEST_ASSERT(tifOk, "TIFF 16-bit export failed");

    std::ifstream tf(tiffPath, std::ios::binary | std::ios::ate);
    APEX_TEST_ASSERT(tf.is_open(), "Could not open generated TIFF");
    size_t tfSize = static_cast<size_t>(tf.tellg());
    APEX_TEST_ASSERT(tfSize > static_cast<size_t>(testW * testH * 6), "TIFF file size smaller than 16-bit uncompressed raster");
    tf.seekg(0);
    uint8_t tHead[4];
    tf.read(reinterpret_cast<char*>(tHead), 4);
    tf.close();
    // Check TIFF header 'II' 0x002A
    APEX_TEST_ASSERT(tHead[0] == 0x49 && tHead[1] == 0x49 && tHead[2] == 0x2A && tHead[3] == 0x00, "TIFF header invalid");
...
bool testColorToneAndDithering() {
    ...
    int gradW = 256;
    int gradH = 16;
    std::vector<apex::FloatRGBA> gradPixels(gradW * gradH);
    for (int y = 0; y < gradH; ++y) {
        for (int x = 0; x < gradW; ++x) {
            float val = 0.3000f + (static_cast<float>(x) / static_cast<float>(gradW)) * (1.0f / 255.0f);
            gradPixels[y * gradW + x] = apex::FloatRGBA(val, val, val);
        }
    }

    std::vector<uint8_t> quantizedNoDither;
    std::vector<uint8_t> quantizedWithDither;
    apex::ExportPipeline::quantizeTo8Bit(gradPixels, gradW, gradH, 0, 0, apex::ColorSpace::sRGB, false, quantizedNoDither);
    apex::ExportPipeline::quantizeTo8Bit(gradPixels, gradW, gradH, 0, 0, apex::ColorSpace::sRGB, true, quantizedWithDither);
    ...
    APEX_TEST_ASSERT(quantizedWithDither.size() == quantizedNoDither.size(), "Dither buffer size mismatch");

    // 2. White Balance & Tone verification
    apex::FloatRGBA neutralPixel(0.5f, 0.5f, 0.5f);
    std::vector<apex::FloatRGBA> inTile(9, neutralPixel);
    std::vector<apex::FloatRGBA> outTile;

    // Test Kelvin change: Warm (3500K) vs Cool (8000K)
    apex::DevelopmentParams warmParams;
    warmParams.kelvin = 3500.0f; // Warm -> Red higher, Blue lower
    apex::ExportPipeline::processTileLinear(inTile, 3, 3, 1, 1, 1, warmParams, outTile);
    APEX_TEST_ASSERT(outTile[0].r > outTile[0].b, "Warm WB should have higher Red gain than Blue");
    std::cout << "  ✓ Warm WB (3500K): R=" << outTile[0].r << ", B=" << outTile[0].b << " (R > B confirmed)\n";

    apex::DevelopmentParams coolParams;
    coolParams.kelvin = 8000.0f; // Cool -> Blue higher, Red lower
    apex::ExportPipeline::processTileLinear(inTile, 3, 3, 1, 1, 1, coolParams, outTile);
    APEX_TEST_ASSERT(outTile[0].b > outTile[0].r, "Cool WB should have higher Blue gain than Red");
    std::cout << "  ✓ Cool WB (8000K): R=" << outTile[0].r << ", B=" << outTile[0].b << " (B > R confirmed)\n";

    // Test Exposure: +1 EV should double linear value
    apex::DevelopmentParams expParams;
    expParams.exposureEV = 1.0f;
    apex::ExportPipeline::processTileLinear(inTile, 3, 3, 1, 1, 1, expParams, outTile);
    float ratio = outTile[0].g / neutralPixel.g;
    APEX_TEST_ASSERT(std::abs(ratio - 2.0f) < 0.1f, "Exposure +1EV should double value");
    ...
    apex::FloatRGBA blownPixel(2.0f, 2.0f, 2.0f);
    std::vector<apex::FloatRGBA> blownTile(9, blownPixel);
    apex::DevelopmentParams hlParams;
    hlParams.highlights = 80.0f;
    apex::ExportPipeline::processTileLinear(blownTile, 3, 3, 1, 1, 1, hlParams, outTile);
    APEX_TEST_ASSERT(outTile[0].r < 2.0f, "Highlights recovery should compress values above knee threshold");
    ...
    apex::FloatRGBA skinPix(0.62f, 0.42f, 0.32f);
    apex::FloatRGBA cyanPix(0.10f, 0.60f, 0.60f);
    std::vector<apex::FloatRGBA> vibTile = { skinPix, skinPix, skinPix, skinPix, skinPix, skinPix, skinPix, skinPix, skinPix };
    apex::DevelopmentParams vibParams;
    vibParams.vibrance = 80.0f;
    apex::ExportPipeline::processTileLinear(vibTile, 3, 3, 1, 1, 1, vibParams, outTile);
...
bool testDetailAndNoiseReduction() {
    ...
    std::vector<apex::FloatRGBA> noisyPadded(paddedSize * paddedSize);
    ...
    for (int y = 0; y < paddedSize; ++y) {
        for (int x = 0; x < paddedSize; ++x) {
            ...
            float nG = noise(rng);
            float nB = noise(rng);
            noisyPadded[y * paddedSize + x] = apex::FloatRGBA(base + nR, base + nG, base + nB);
        }
    }

    // Process with Noise Reduction
    apex::DevelopmentParams nrParams;
    nrParams.luminanceNR = 70.0f;
    nrParams.chromaNR = 80.0f;
    nrParams.sharpeningAmount = 50.0f;
    nrParams.sharpeningMasking = 30.0f;

    std::vector<apex::FloatRGBA> filteredTile;
    apex::ExportPipeline::processTileLinear(noisyPadded, paddedSize, paddedSize, size, size, pad, nrParams, filteredTile);
    ...
    APEX_TEST_ASSERT(varAfter < varBefore * 0.5f, "Noise variance should be significantly reduced by Bilateral NR");
    ...
    APEX_TEST_ASSERT(edgeAfter >= 0.35f, "Edge contrast must be preserved and sharp");
...
bool testMonochromeMode() {
    ...
    apex::ExportPipeline pipeline;
    apex::DevelopmentParams monoParams;
    ...
    apex::ExportOptions opt;
    opt.format = apex::ExportFormat::JPEG;
    opt.jpegQuality = 95;
    opt.tileSize = 512;
    opt.tilePadding = 16;
    ...
    APEX_TEST_ASSERT(ok, "Monochrome export failed");

    // Verify generated pixels satisfy R == G == B
    std::vector<apex::FloatRGBA> inTile = {
        apex::FloatRGBA(0.8f, 0.1f, 0.1f), // Pure Red
        apex::FloatRGBA(0.1f, 0.1f, 0.8f), // Pure Blue
        apex::FloatRGBA(0.1f, 0.8f, 0.1f), // Pure Green
        apex::FloatRGBA(0.8f, 0.8f, 0.1f)  // Pure Yellow
    };
    std::vector<apex::FloatRGBA> outTile;
    apex::ExportPipeline::processTileLinear(inTile, 2, 2, 2, 2, 0, monoParams, outTile);

    for (size_t i = 0; i < outTile.size(); ++i) {
        APEX_TEST_ASSERT(std::abs(outTile[i].r - outTile[i].g) < 1e-5f, "Monochrome output must have R == G");
        APEX_TEST_ASSERT(std::abs(outTile[i].g - outTile[i].b) < 1e-5f, "Monochrome output must have G == B");
    }
    ...
    APEX_TEST_ASSERT(redFilteredVal > blueFilteredVal * 2.0f, "Red filter should enhance red and darken blue sky");
...
bool testExifPreservation() {
    ...
    std::vector<uint8_t> payload = apex::ImageWriter::buildExifPayload(meta);
    APEX_TEST_ASSERT(payload.size() > 100, "Exif payload too small");
    ...
    APEX_TEST_ASSERT(containsString("Sony"), "Exif Make missing in payload");
    APEX_TEST_ASSERT(containsString("ILCE-7RM5"), "Exif Model missing in payload");
    APEX_TEST_ASSERT(containsString("FE 24-70mm F2.8 GM II"), "Exif LensModel missing in payload");
    APEX_TEST_ASSERT(containsString("2026:09:20 12:30:45"), "Exif DateTimeOriginal missing in payload");
...
bool testTileRenderingOOMAvoidance() {
    ...
    opt.format = apex::ExportFormat::JPEG;
    opt.jpegQuality = 92;
    opt.chromaSubsampling = apex::ChromaSubsampling::YUV420;
    ...
    APEX_TEST_ASSERT(ok, "High-resolution tile rendering export failed");

    std::ifstream bf(bigOutPath, std::ios::binary | std::ios::ate);
    APEX_TEST_ASSERT(bf.is_open(), "Generated 45MP JPEG does not exist");
...
int main() {
    std::cout << "=======================================================\n";
    std::cout << "  PROJECT: APEX FIELD - PHASE 1 VERIFICATION SUITE\n";
    std::cout << "=======================================================\n";
